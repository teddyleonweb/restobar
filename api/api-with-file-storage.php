<?php
/**
 * API Completa para Tu Bar Resto con almacenamiento de archivos
...
function upload_and_process_image($file, $restaurant_id, $category = 'other') {
    ...
    // Validar formato
    $allowed_formats = json_decode($settings->allowed_formats, true);
    ...
    if ($settings->generate_thumbnails && $image_info) {
        $thumbnail_sizes = json_decode($settings->thumbnail_sizes, true);
        ...
        foreach ($thumbnail_sizes as $size_name => $dimensions) {
            ...
            $thumb_result = resize_image($file_path, $thumb_path, $dimensions[0], $dimensions[1], $settings->image_quality);
