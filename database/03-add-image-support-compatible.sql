...
-- Script compatible con MySQL 5.7+ (sin valores por defecto en JSON)
-- =====================================================
...
CREATE TABLE IF NOT EXISTS kvq_tubarresto_restaurant_images (
    ...
    -- Metadatos adicionales (sin valor por defecto)
...
-- 5. CREAR TABLA PARA CONFIGURACIÓN DE UPLOAD (SIN VALORES POR DEFECTO EN JSON)
CREATE TABLE IF NOT EXISTS kvq_tubarresto_upload_settings (
    ...
    allowed_formats TEXT COMMENT 'Formatos permitidos separados por coma: image/jpeg,image/png,image/webp,image/gif',
    ...
    thumbnail_sizes TEXT COMMENT 'Tamaños de miniaturas en formato: thumbnail:150x150,medium:300x300,large:800x800',
...
-- 10. CREAR FUNCIONES AUXILIARES PARA MANEJAR CONFIGURACIONES

-- Función para obtener formatos permitidos como array
DROP FUNCTION IF EXISTS get_allowed_formats;

DELIMITER $$

CREATE FUNCTION get_allowed_formats(restaurant_id_param INT)
RETURNS TEXT
READS SQL DATA
DETERMINISTIC
BEGIN
    DECLARE formats_text TEXT DEFAULT '';
    
    SELECT COALESCE(allowed_formats, 'image/jpeg,image/png,image/webp,image/gif')
    INTO formats_text
...
-- Función para obtener tamaños de thumbnails
DROP FUNCTION IF EXISTS get_thumbnail_sizes;

DELIMITER $$

CREATE FUNCTION get_thumbnail_sizes(restaurant_id_param INT)
RETURNS TEXT
READS SQL DATA
DETERMINISTIC
BEGIN
    DECLARE sizes_text TEXT DEFAULT '';
    
    SELECT COALESCE(thumbnail_sizes, 'thumbnail:150x150,medium:300x300,large:800x800')
    INTO sizes_text
    FROM kvq_tubarresto_upload_settings 
    WHERE restaurant_id = restaurant_id_param;
